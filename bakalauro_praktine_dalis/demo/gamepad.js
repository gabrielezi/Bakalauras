...
const Gamepad = class {
  ...
  onRequest(request, back, old) {
    ...
    if (!this.blocklyManager.isCoding && this.highlight)
      this.blocklyManager.setHighlight(request.id);
    ...
    if (
      request.method == Gamepad["STATES"]["STARTED"] &&
...
Gamepad.utils = {
  ...
  blocks: function (blocks) {
    ...
    for (let type in blocks) {
      ...
      if (hastemplate && !hasStatements)
        throw new Error("a template block require at least a statement");
      if (!hastemplate && hasStatements)
        throw new Error("statements setted without the template");

      // load the javascript
      Blockly.JavaScript["" + type] = Gamepad.utils.js(
        block.method,
        block.args,
        block.order,
        block.template,
  ...
  wrap: function (blocks) {
    ...
    for (let type of blocks) {
      if (!(type in Blockly.JavaScript))
        throw new Error("The following type does not exist: " + type);
...
Gamepad.init = function (options) {
  ...
  // load the toolbox
  if (options.hasOwnProperty("toolbox"))
    this["TOOLBOX"] = Gamepad.utils.xml2json(options.toolbox, "");
  // load the context
  if (options.hasOwnProperty("context")) this["CONTEXT"] = options.context;

  // load the blocks
  if (options.hasOwnProperty("blocks"))
    Gamepad.utils.blocks(options.blocks, "");
  // wrap standar blocks
  ...
  clear() {
    ...
    for (let block of blocks) {
      if (
        ...
        (!this.start || block.type != Gamepad["BLOCKS"]["START"])
      )
        block.dispose(false);
    }
  }
  ...
  removeHighlight() {
    if (this.customHighlight) {
      ...
      for (let i = blocks.length; i > 0; i--)
        blocks[i - 1].classList.remove("blocklySelected");
    } else {
  ...
  const asynchronizer = new Gamepad.Asynchronizer(
    {
      ...
      freeQueue: function () {
        ...
        requests.forEach(
          (request) => request && request.resolve && request.resolve(false)
        );
      },
      // start the worker
      start: function (back) {
      ...
      const start = async () => {
        try {
          while (true) {
            ...
            // backward or forward
            request.back ? await backward() : await forward();
      ...
      const forward = async () => {
        ...
        if (this.history.current) {
          ...
          if (this.history.current.method == Gamepad["STATES"]["FINISHED"]) {
            if (
              request.request.method != Gamepad["STATES"]["STARTED"] &&
              request.request.method != Gamepad["STATES"]["COMPLETED"]
            ) {
...
Gamepad.observer = {
  ...
  prepareArray: function (source, observer) {
    ...
    while (l--) {
      ...
      if (
        item &&
        typeof item === "object" &&
        !Gamepad.observer.nonObservables.hasOwnProperty(item.constructor.name)
      ) {
  ...
  loadChange: function (observer, change) {
    ...
    if (Array.isArray(target)) {
      if (change.type == Gamepad.observer.INSERT) {
        ...
        if (target.length > key) target.splice(key, 0, change.value);
  ...
  unloadChange: function (observer, change) {
    ...
    if (Array.isArray(target)) {
      if (change.type == Gamepad.observer.INSERT) {
        // if the key isn't a number the if is false and it will set the value correctly
        if (target.length > key) target.splice(key, 1);
        else delete target[key];
      }
      if (change.type == Gamepad.observer.DELETE)
        target.splice(key, 0, change.oldValue);
    } else {
    ...
    if (change.type == Gamepad.observer.SHIFT)
      target[key].unshift(change.oldValue);
    if (change.type == Gamepad.observer.REVERSE) target[key].reverse();
    // common changes
  ...
  observeJson: function (target) {
    if (
      ...
      if (!target || typeof target !== "object") {
        ...
        Gamepad.observer.nonObservables.hasOwnProperty(target.constructor.name)
      ) {
        throw new Error(
          target +
...
Gamepad.Observer = class {
  constructor(properties, cloningFunction) {
    ...
    if (properties.parent === null) {
      ...
      Object.defineProperties(
        targetClone,
        Gamepad.observer.observableDefinition
      );
    } else {
  ...
  set(target, key, value) {
    ...
    if (
      value &&
      typeof value === "object" &&
      !Gamepad.observer.nonObservables.hasOwnProperty(value.constructor.name)
    ) {
  ...
  get(target, key) {
    ...
    const proxiedArrayMethods = {
      ...
      fill: function proxiedFill(target, observed) {
        ...
        start =
          argLen < 2 ? 0 : normArgs[1] < 0 ? tarLen + normArgs[1] : normArgs[1];
        end =
          argLen < 3
            ? tarLen
            : normArgs[2] < 0
...
Gamepad.Store = class {
  ...
  unloadChanges() {
    ...
    if (this.index > -1) {
      ...
      // unload all changes
      while (--i >= 0) Gamepad.observer.unloadChange(this.observer, changes[i]);
